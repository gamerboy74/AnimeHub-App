...
import { animeAPI, episodeAPI, reviewAPI, userAPI, AnimeWithStats, Episode, Review } from '../lib/supabase';
    ...
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.actionRow}>
          <TouchableOpacity style={styles.playBigBtn} onPress={() => router.push(`/anime/episodes/${animeId}`)}>
...
function EpisodesTab({ episodes, anime, router, user }: any) {
  const freeEps = episodes.filter((e: Episode) => !e.is_premium);
  const premiumEps = episodes.filter((e: Episode) => e.is_premium);
        ...
        <>
          ...
          {episodes.length > 5 && (
            <TouchableOpacity
              ...
              onPress={() => router.push(`/anime/episodes/${anime.id}`)}
            >
    ...
    <View style={styles.tabContent}>
      {user && (
        <TouchableOpacity
          ...
          onPress={() => router.push(`/anime/reviews/${anime.id}`)}
        >
